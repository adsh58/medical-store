...
async def list_expiry_alerts(db: AsyncSession = Depends(get_db)):
    """
    List all pending and active medicine expiry warning notifications.
    """
    return await expiry_repo.get_pending_alerts(db)
...
async def trigger_expiry_scan(
    ...
    """
    alerts = await expiry_service.check_expiry_dates(db)
    await db.commit()
    ...
    if alerts:
        ...
        query = select(ExpiryTracking).filter(ExpiryTracking.id.in_(alert_ids)).options(
...
async def resolve_expiry_alert(
    ...
    """
    alert = await expiry_repo.get(db, id)
    if not alert:
